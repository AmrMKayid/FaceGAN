...
def main():
  ...
  parser.add_argument(
      '--scale_mask',
      default=1.4,  #TODO: experiment with this
  ...
  tflib.init_tf()
  with dnnlib.util.open_url(args.model_url,
                            cache_dir=config.cache_dir) as model:
    generator_network, discriminator_network, Gs_network = pickle.load(model)
  ...
  for images_batch in tqdm(split_to_batches(ref_images, args.batch_size),
    ...
    for img_array, dlatent, img_path, img_name in zip(
      ...
      if args.composite_mask and mask_img is not None \
      ...
      np.save(
          os.path.join(args.dlatent_dir, f'{img_name}.npy'),
          dlatent,
      )
