...
def _upfirdn_2d_ref(x, k, upx, upy, downx, downy, padx0, padx1, pady0, pady1):
  """Slow reference implementation of `upfirdn_2d()` using standard TensorFlow
  ops."""
...
def upsample_conv_2d(x,
  ...
  if data_format == 'NCHW':
    ...
    output_shape = [
        _shape(x, 0), outC, (_shape(x, 2) - 1) * factor + convH,
    ...
    output_shape = [
        _shape(x, 0), (_shape(x, 1) - 1) * factor + convH,
        (_shape(x, 2) - 1) * factor + convW, outC
    ]
