...
def create_variable_for_generator(
    name,
    batch_size,
    tiled_dlatent,
    model_scale=18,
  ...
  if tiled_dlatent:
    low_dim_dlatent = tf.get_variable(
        'learnable_dlatents',
...
class Generator:
    ...
    if tiled_dlatent:
      ...
      if custom_input is not None:
        model.components.synthesis.run(
            ...
            minibatch_size=self.batch_size,
            custom_inputs=[
                partial(custom_input.eval(), batch_size=batch_size),
                partial(create_stub, batch_size=batch_size)
            ],
            structure='fixed',
        )
  ...
  def set_dlatents(self, dlatents):
    if self.tiled_dlatent:
      if (dlatents.shape !=
          (self.batch_size, 512)) and (dlatents.shape[1] != 512):
        dlatents = np.mean(dlatents, axis=1)
      if (dlatents.shape != (self.batch_size, 512)):
        dlatents = np.vstack(
            [dlatents,
             np.zeros((self.batch_size - dlatents.shape[0], 512))])
