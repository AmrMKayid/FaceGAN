...
class RunContext(object):
  ...
  def __init__(self,
    ...
    if config_module is not None:
      with open(os.path.join(submit_config.run_dir, "config.txt"), "w") as f:
        filtered_dict = {
            ...
            not isinstance(v, (types.ModuleType, types.FunctionType,
                               types.LambdaType, submit.SubmitConfig, type))
        }
    ...
    with open(os.path.join(submit_config.run_dir, "run.txt"), "w") as f:
      pprint.pprint(self.run_txt_data,
                    stream=f,
                    indent=4,
                    width=200,
                    compact=False)
  ...
  def update(self, loss: Any = 0, cur_epoch: Any = 0,
             max_epoch: Any = None) -> None:
  ...
  def should_stop(self) -> bool:
    """Tell whether a stopping condition has been triggered one way or
    another."""
  ...
  def close(self) -> None:
    ...
    if not self.has_closed:
      ...
      self.run_txt_data["stop_time"] = datetime.datetime.now().isoformat(
          sep=" ")
