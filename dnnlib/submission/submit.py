...
class SubmitConfig(util.EasyDict):
  ...
  def __init__(self):
    ...
    self.run_desc = ""
    self.run_dir_ignore = [
        "__pycache__", "*.pyproj", "*.sln", "*.suo", ".cache", ".idea", ".vs",
        ".vscode"
    ]
...
def get_path_from_template(path_template: str,
                           path_type: PathType = PathType.AUTO) -> str:
  """Replace tags in the given path template and return either Windows or Linux
  formatted path."""
...
def convert_path(path: str, path_type: PathType = PathType.AUTO) -> str:
  """Convert a normal path to template and the convert it back to a normal path
  with given path type."""
...
def _create_run_dir_local(submit_config: SubmitConfig) -> str:
  ...
  submit_config.run_name = "{0:05d}-{1}".format(submit_config.run_id,
...
def _get_next_run_id_local(run_dir_root: str) -> int:
  """Reads all directory names in a given directory (non-recursive) and returns
  the next (increasing) run id.

  Assumes IDs are numbers at the start of the directory names.
  """
  dir_names = [
      d for d in os.listdir(run_dir_root)
...
def _populate_run_dir(run_dir: str, submit_config: SubmitConfig) -> None:
  """Copy all necessary files into the run dir.
  ...
  files += [(os.path.join(dnnlib_module_dir_path, "submission", "_internal",
                          "run.py"), os.path.join(run_dir, "run.py"))]

  util.copy_files_and_create_dirs(files)

  pickle.dump(submit_config,
...
def run_wrapper(submit_config: SubmitConfig) -> None:
  ...
  try:
    print("dnnlib: Running {0}() on {1}...".format(submit_config.run_func_name,
...
def submit_run(submit_config: SubmitConfig, run_func_name: str,
  ...
  """Create a run dir, gather files related to the run, copy files to the run
  dir, and launch the run in appropriate place."""
  ...
  if submit_config.submit_target in {SubmitTarget.LOCAL}:
    ...
    submit_config.task_name = "{0}-{1:05d}-{2}".format(submit_config.user_name,
