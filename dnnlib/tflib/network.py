...
class Network:
  ...
  def reset_trainables(self) -> None:
    """Re-initialize all trainable variables of this network, including sub-
    networks."""
  ...
  def get_output_for(self,
    ...
    with tfutil.absolute_variable_scope(self.scope,
                                        reuse=True), tf.name_scope(self.name):
      assert tf.get_variable_scope().name == self.scope
  ...
  def copy_own_vars_from(self, src_net: "Network") -> None:
    """Copy the values of all variables from the given network, excluding sub-
    networks."""
  ...
  def copy_vars_from(self, src_net: "Network") -> None:
    """Copy the values of all variables from the given network, including sub-
    networks."""
  ...
  def copy_trainables_from(self, src_net: "Network") -> None:
    ...
    names = [
        name for name in self.trainables.keys() if name in src_net.trainables
    ]
    tfutil.set_vars(
        tfutil.run({self.vars[name]: src_net.vars[name] for name in names}))
  ...
  def convert(self,
    ...
    """Create new network with the given parameters, and copy all variables
    from this network."""
    if new_name is None:
      new_name = self.name
  ...
  def run(self,
    ...
    if key not in self._run_cache:
      with tfutil.absolute_name_scope(self.scope +
                                      "/_Run"), tf.control_dependencies(None):
        if custom_inputs is not None:
          with tf.device("/gpu:0"):
            in_expr = [
                input_builder(name)
                for input_builder, name in zip(custom_inputs, self.input_names)
            ]
        ...
        for gpu in range(num_gpus):
          with tf.device("/gpu:%d" % gpu):
            ...
            if input_transform is not None:
              ...
              in_gpu = [in_gpu
                       ] if tfutil.is_tf_expression(in_gpu) else list(in_gpu)
            ...
            if output_transform is not None:
              ...
              out_gpu = [out_gpu
                        ] if tfutil.is_tf_expression(out_gpu) else list(out_gpu)
    ...
    for mb_begin in range(0, num_items, minibatch_size):
      ...
      mb_in = [
          src[mb_begin:mb_end] if src is not None else np.zeros([mb_num] +
                                                                shape[1:])
  ...
  def list_layers(self) -> List[Tuple[str, TfExpression, List[TfExpression]]]:
    """Returns a list of (layer_name, output_expr, trainable_vars) tuples
    corresponding to individual layers of the network.

    Mainly intended to be used for reporting.
    """
    layers = []
  ...
  def setup_weight_histograms(self, title: str = None) -> None:
    """Construct summary ops to include histograms of all trainable parameters
    in TensorBoard."""
    if title is None:
      title = self.name
